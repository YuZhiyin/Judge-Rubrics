...
def setup_tokenizer(tokenizer_name_or_path="gpt2", max_length=4096):
    ...
    if tok.pad_token_id is None:
        # Prefer adding an explicit PAD token to avoid collisions with EOS/CLS usage.
        print("Tokenizer missing pad token; adding new pad token '[PAD]'.")
    ...
    cls_id = None
    # Prefer an explicit CLS token; add one if it's missing to avoid EOS/CLS ambiguity.
    if not hasattr(tok, "cls_token_id") or tok.cls_token_id is None:
        print("Tokenizer missing CLS token; adding new CLS token '[CLS]'.")
        tok.add_special_tokens({'cls_token': '[CLS]'})

    if hasattr(tok, "cls_token_id") and tok.cls_token_id is not None:
        cls_id = tok.cls_token_id
        print(f"Using CLS token ID ({cls_id}) as CLS_ID (effective).")
    elif hasattr(tok, "bos_token_id") and tok.bos_token_id is not None:
        ...
        print(f"Warning: CLS/BOS token not found. Using EOS token ID ({cls_id}) as CLS_ID (effective).")
    else:
        print("Error: Tokenizer has no CLS/BOS/EOS token. Cannot determine CLS_ID for sequence representation.")
        exit(1)

    # If PAD and CLS collide (common for GPT2 when EOS is reused), enforce distinct CLS.
    if pad_id is not None and cls_id == pad_id:
        print("PAD_ID equals CLS_ID; adding explicit CLS token '[CLS]' to avoid masking the CLS position.")
        tok.add_special_tokens({'cls_token': '[CLS]'})
...
def evaluate(model, loader, device, autocaster, eval_type="Validation"):
    ...
    for idsL, maskL, labL in pbar_eval:
        for ids, mask, lab in zip(idsL, maskL, labL):
            ...
            rand_idx = random.randint(0, lab.numel() - 1)
            if lab.numel() > 0 and lab[rand_idx].item() == 1:
                naive_correct += 1
